...
from models.layers import rms_norm, LinearSwish, SwiGLU, Attention, RotaryEmbedding, CosSin, CastedEmbedding, CastedLinear, SSM
...
class TinyRecursiveReasoningModel_ACTV1Config(BaseModel):
    ...
    # for ablation with SSM blocks
    ssm_t: bool = False # use ssm on L instead of transformer
    ssm_inner_size: int
    ssm_state_size: int
    ssm_dt_rank: int
    ssm_kernel_size: int
...
class TinyRecursiveReasoningModel_ACTV1Block(nn.Module):
    def __init__(self, config: TinyRecursiveReasoningModel_ACTV1Config) -> None:
        ...
        if self.config.mlp_t:
            ...
            self.ssm = SSM(
                ...
                proj_bias=config.ssm_proj_bias,
    ...
    def forward(self, cos_sin: CosSin, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.config.mlp_t:
            ...
            hidden_states = rms_norm(hidden_states + self.ssm(cos_sin=cos_sin, hidden_states=hidden_states), variance_epsilon=self.norm_eps)
